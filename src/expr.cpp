...
#define OPLIST_UNARY(X) \
	X(NOT, {"!"}, RIGHT, 15, !x) X(BITNOT, {"~"}, RIGHT, 15, ~(long long)x) X(LOG10, (vstr{"log10", "log"}), RIGHT, 15, log10(x)) X(LOG2, {"log2"}, RIGHT, 15, log2(x)) X(LN, {"ln"}, RIGHT, 15, log(x)) X(SQRT, {"sqrt"}, RIGHT, 15, sqrt(x)) X(SIN, {"sin"}, RIGHT, 15, sin(x)) X(COS, {"cos"}, RIGHT, 15, cos(x)) X(CTG, (vstr{"ctg", "cot"}), RIGHT, 15, cos(x) / sin(x)) X(TG, (vstr{"tan", "tg"}), RIGHT, 15, sin(x) / cos(x)) X(SEC, {"sec"}, RIGHT, 15, 1.0 / cos(x)) X(CSC, (vstr{"cosec", "csc" }), RIGHT, 15, 1.0 / sin(x)) X(ARCSIN, (vstr{ "arcsin", "asin" }), RIGHT, 15, asin(x)) X(ARCCOS, (vstr{ "arccos", "acos" }), RIGHT, 15, acos(x)) X(ARCCTG, (vstr{ "arcctg", "arccot", "actg", "acot" }), RIGHT, 15, atan(1.0 / x)) X(ARCTG, (vstr{ "arctg", "arctan", "atg", "atan" }), RIGHT, 15, atan(x)) X(ARCSEC, (vstr{ "arcsec", "asec" }), RIGHT, 15, acos(1.0 / x)) X(ARCCSC, (vstr{ "arccsc", "arccosec", "acsc", "acosec" }), RIGHT, 15, asin(1.0 / x)) X(FLOOR, {"floor"}, RIGHT, 15, floor(x)) X(TRUNC, {"trunc"}, RIGHT, 15, trunc(x)) X(CEIL, {"ceil"}, RIGHT, 15, ceil(x)) X(ABS, {"abs"}, RIGHT, 15, abs(x)) X(ROUND, {"round"}, RIGHT, 15, round(x)) X(ERF, {"erf"}, RIGHT, 15, erf(x)) X(ERFC, {"erfc"}, RIGHT, 15, erfc(x)) X(TGAMMA, {"tgamma"}, RIGHT, 15, tgamma(x)) X(LGAMMA, {"lgamma"}, RIGHT, 15, lgamma(x)) X(SGN, {"sgn"}, RIGHT, 15, (0 < x) - (x < 0))
// Binary
#define OPLIST_BINARY(X) \
	...
	X(EQ, {"=="}, LEFT, 9, x == y) X(NEQ, {"!="}, LEFT, 9, x != y) \
	X(LT, {"<"}, LEFT, 10, x < y) X(LEQ, {"<="}, LEFT, 10, x <= y) X(GT, {">"}, LEFT, 10, x > y) X(GEQ, {">="}, LEFT, 10, x >= y) \
	X(SPC, {"<=>"}, LEFT, 11, x < y ? -1 : (x > y ? 1 : 0)) \
	X(SHL, {"<<"}, LEFT, 12, (long long)x << (int)y) X(SHR, {">>"}, LEFT, 12, (long long)x >> (int)y) \
	X(ADD, {"+"}, LEFT, 13, x + y) X(SUB, {"-"}, LEFT, 13, x - y) \
	X(MUL, {"*"}, LEFT, 14, x * y) X(DIV, {"/"}, LEFT, 14, x / y) X(MOD, {"%"}, LEFT, 14, fmod(x, y)) X(POW, {"**"}, LEFT, 14, pow(x, y)) X(FDIV, {"//"}, LEFT, 14, trunc(x / y))
// Ternary
#define OPLIST_TERNARY(X) \
	X(QUESTION, {"?"}, RIGHT, 2, NAN) X(COLON, {":"}, RIGHT, 2, x ? y : z)
// All
#define OPLIST(X) \
	OPLIST_UNARY(X) \
	OPLIST_BINARY(X) \
...
std::unordered_map<expr_optype, int> optype2prec = {
	{ OP_NEG, 15 }, { OP_POS, 15 }, { OP_LPAREN, 16 },
...
std::unordered_map<expr_optype, int> optype2assoc = {
	{ OP_NEG, RIGHT }, { OP_POS, RIGHT }, { OP_LPAREN, LEFT },
...
std::unordered_map<expr_optype, int> optype2ar = {
	{ OP_NEG, 1 }, { OP_POS, 1 }, { OP_LPAREN, 0 }, { OP_QUESTION, 0 }, { OP_COLON, 3 },
